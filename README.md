...
- **Charts render for the common types.** Bar/column, line and pie charts — including bar + line combos on one or two value axes — are drawn from the series data cached in the file (no recalculation). Other types (scatter, area, 3-D, % stacked) show a labelled placeholder at the chart's position and size.
