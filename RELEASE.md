...
- Formulas: if the source file was generated by a tool that does not pre-calculate cell values (e.g. openpyxl, ExcelJS without `calc()`), the preview shows the formula text (`=A1+B1`) instead of the computed result. Files saved by Excel or LibreOffice always include cached values and render correctly.
