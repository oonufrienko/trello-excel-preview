...
### Added
- **Embedded charts render.** Bar/column (clustered and stacked, vertical and horizontal), line, and pie/doughnut charts are drawn as SVG from the series data Excel caches inside `chartN.xml` (`numCache`/`strCache`) — no formula or range evaluation involved. Bar + line combos render as one plot, including the case where the line sits on its own secondary value axis. Everything else (scatter, area, 3-D, % stacked) shows a labelled placeholder at the chart's exact position and size, the same pattern already used for WMF images: a chart that cannot be drawn says so rather than vanishing. Positioning reuses the existing `parseAnchors`/`positionImages` pipeline.
